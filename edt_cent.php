...
<br />
    <?php
        if($_POST['btn']){
            ...
            $result = json_decode(upCentro($params));
            //print_r($result);
            if($result->result === "sucess"){
                echo '<script>alert("Centro de Custos Atualizado.")</script>';
            }else{ 
        ...
        if($_POST['del']){
            $prms = ['action' => '25', 'idcent' => $_POST['idcent']];
            $del = loadApi($prms);
        ...
        $parameters = ['idcentro' => $idcentro, 'action' => '20'];
        $cons = json_decode(listaCentros($parameters));
        $res = $cons->datas;
        //print_r($res);
        ...
        $params = ['action' => '5'];
        $resu = json_decode(listaEmpresas($params));

            if($resu->result === "sucess"){
                $emp = $resu->datas;
                //print_r($emp);
            }else{
                echo '<script>alert('.$resu->error.')</script>';
            }
        ?>
	<div class="container">
    ...
    <div class="col-sm-9">
                <div class="contact-form">
                    <h2 class="title text-center">Editar Centro de Custos</h2>
                    ...
                    <form id="main-contact-form" class="contact-form row text-left" name="contact-form" method="post">
                        <div class="form-group">
                            ...
                            <?php
                                foreach($emp as $item){
                                ?>
                                  <option value="<?=$item->idemp?>" <?=($res[0]->empresa == $item->nome_fantasia)?'selected':''?>><?=$item->tipo_empresa.' - '.$item->nome_fantasia?></option>
                                <?php 
                                }
                                ?>
                            </select>
                        ...
                        <!-- <label style="float: left; margin-left: 2%;">ISS</label> -->
                            <input type="text" name="centro" id="centro" class="form-control"  maxlength="200" value="<?=$res[0]->centro?>">
                        </div>  
                        <div class="form-group">
                        <!-- <label style="float: left; margin-left: 2%;">ISS</label> -->
                            <input type="text" name="obs" id="message" maxlength="200" class="form-control"  value="<?=$res[0]->obs?>">
                        </div>  
                        <input type="hidden" name="idcent" value="<?=$res[0]->idcentro?>" />
                        <input type="hidden" name="empresa" value="<?=$res[0]->fk_empresa?>" />
                                              
                        <div class="form-group">
                        <a class="btn btn-info" href="javascript:location.href = 'busca_cent.php'">Voltar</a>
